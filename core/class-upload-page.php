...
class Fontiran_Upload_Page extends WP_Fontiran_Admin_Page {
	...
	protected function set_information() {
		...
		$this->font['active'] = '1';
		$this->font['name'] = (!empty($this->post['font_name'])) ? wp_strip_all_tags( $this->post['font_name'] ) : null;
		$this->font['weight'] = (!empty($this->post['font_weight'])) ? $this->fontWeight( $this->post['font_weight'] ) : 'normal';
		$this->font['style'] = (!empty($this->post['font_style'])) ? $this->fontStyle( $this->post['font_style'] ) : 'normal';
	...
	public function is_zip() {	
		...
		return preg_match("/zip$/i",$name) ? true : false;
	}
